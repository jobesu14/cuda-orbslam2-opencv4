...
void TemplatedVocabulary<TDescriptor,F>::saveToBinaryFile(const std::string &filename) const {
  ...
  fstream f;
  f.open(filename.c_str(), ios_base::out|ios::binary); //open file for writing in binary mode

  unsigned int nb_nodes = m_nodes.size(); //number of three nodes
  ...
  for(size_t i=1; i<nb_nodes;i++) {
	...
	f.write((char*)&is_leaf, sizeof(is_leaf)); // i put this one at the end for alignment....
